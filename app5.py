    ...
    <script>
        ...
        function loadAssociationRules() {
            fetch('/api/association-rules')
                .then(response => response.json())
                .then(data => {
                    let html = '<table><thead><tr><th>If Customer Buys</th><th>Then Also Buys</th><th>Confidence</th><th>Lift</th></tr></thead><tbody>';
...
def get_recommendations(entity_type, entity_id):
    ...
    try:
        ...
        if entity_type == 'customer':
            ...
            # Find similar customers (bought at least one same product)
            ...
            vendor_products = set(products[products['Vendor_ID'] == entity_id]['Product_ID'].values)
            
            # Find customers who bought from this vendor
            vendor_customers = licenses[licenses['Product_ID'].isin(vendor_products)]['Customer_ID'].unique()
            
            # Find what other products these customers bought
            recommended_products = licenses[
        ...
        for prod_id, count in recommended_products.items():
            product_info = products[products['Product_ID'] == prod_id].iloc[0]
...
def association_rules_api():
    ...
    try:
        ...
        transactions = licenses.groupby('Customer_ID')['Product_ID'].apply(list).values
        ...
        if len(frequent_itemsets) > 0:
            ...
            for _, rule in rules.iterrows():
                antecedent = ', '.join([product_names.get(p, p)[:20] for p in rule['antecedents']])
                consequent = ', '.join([product_names.get(p, p)[:20] for p in rule['consequents']])
...
def driver_analysis():
    ...
    try:
        licenses = data['licenses']
        renewal_history = data['renewal_history']
        
        # Merge to get renewal status
        licenses_with_renewal = licenses.merge(
            renewal_history[['License_ID', 'Renewal_Status']],
            on='License_ID',
            how='left'
        )
        
        # Create binary renewal outcome
        licenses_with_renewal['Renewed'] = (
            licenses_with_renewal['Renewal_Status'] == 'Completed'
        ).astype(int)
        
        # Fill missing renewals as 0
        licenses_with_renewal['Renewed'] = licenses_with_renewal['Renewed'].fillna(0)
        ...
        # Prepare data
        X = licenses_with_renewal[driver_cols].fillna(0)
        y = licenses_with_renewal['Renewed']
        ...
        return jsonify({
            'drivers': [d[0] for d in driver_importance],
